...
--CREATE OR REPLACE VIEW BdUsuariosNodos AS
SELECT DISTINCT
    E.SPRIDEN_ID ||'.PEPN01.UPN.'||
        ...
        C.SSRATTR_ATTR_CODE  ||'.'|| A.SFRSTCR_TERM_CODE AS EXTERNAL_ASSOCIATION_KEY
    , 'PEPN01.UPN.' ||
        CASE SUBSTR(A.SFRSTCR_TERM_CODE,4,1)
            WHEN '3' THEN 'PN'
            WHEN '4' THEN 'UG'
            ...
            ELSE 'EP' END ||'.'||
        CASE WHEN B.SSBSECT_CAMP_CODE IN ('TML','TSI') THEN 'TRU' ELSE B.SSBSECT_CAMP_CODE END ||'.'||
        C.SSRATTR_ATTR_CODE  ||'.'|| A.SFRSTCR_TERM_CODE AS EXTERNAL_NODE_KEY
    , A.SFRSTCR_PIDM AS EXTERNAL_USER_KEY
    , CASE SUBSTR(A.SFRSTCR_TERM_CODE,4,1) --'UPN.<Instancia>.Banner.<Nivel>'
        ...
        ELSE 'UPN.UsuarioNodo.Banner.EPEC' END AS DATA_SOURCE_KEY
