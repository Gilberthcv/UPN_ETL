...
AS '
	...
	, PLAN_ESTUDIOS AS (
		...
		FROM LOE_PROGRAM_AREA_PRIORITY A
				...
				LEFT JOIN UPN_RPT_DM_DEV.ODSMGR.COURSE_CATALOG D ON B.TERM_CODE_EFF = D.ACADEMIC_PERIOD AND CASE WHEN B.AREA_RULE IS NULL THEN B.SUBJ_CODE ELSE SMRARUL_SUBJ_CODE END = D.SUBJECT
	...
	, MATRICULADOS AS (
		...
		FROM SFRSTCR A
				...
				LEFT JOIN UPN_RPT_DM_DEV.ODSMGR.COURSE_CATALOG E ON SSBSECT_TERM_CODE = E.ACADEMIC_PERIOD AND SSBSECT_SUBJ_CODE = E.SUBJECT AND SSBSECT_CRSE_NUMB = E.COURSE_NUMBER
		WHERE SFRSTCR_RSTS_CODE NOT IN (''DD'',''DW'') AND SSBSECT_SUBJ_CODE NOT IN (''TUTO'',''REPS'',''XSER'',''ACAD'',''TEST'',''XPEN'') AND (SSBSECT_SUBJ_CODE <> ''IDIO'' OR SFRSTCR_LEVL_CODE <> ''CR'')
			AND B.FECHA_INICIO = (SELECT MAX(B1.START_DATE) FROM SFRSTCR A1 INNER JOIN LOE_SECTION_PART_OF_TERM B1 ON A1.SFRSTCR_TERM_CODE = B1.TERM_CODE WHERE A1.SFRSTCR_PIDM = A.SFRSTCR_PIDM)
			--AND SFRSTCR_TERM_CODE IN (SELECT TERM_CODE FROM LOE_SECTION_PART_OF_TERM WHERE CURRENT_DATE BETWEEN START_DATE AND END_DATE AND SUBSTR(TERM_CODE,4,1) IN (''4'',''5''))
		)
	, PROYECCIONES AS (
		...
		FROM SFRREGP A
				...
				INNER JOIN LOE_PROGRAM_OVERALL_RESULTS C ON SFRREGP_PIDM = C.PERSON_UID AND SFRREGP_PROGRAM = C.PROGRAM
																AND C.SEQUENCE_NUMBER = (SELECT MAX(C1.SEQUENCE_NUMBER) FROM LOE_PROGRAM_OVERALL_RESULTS C1
																						WHERE C1.PERSON_UID = C.PERSON_UID AND C1.PROGRAM = C.PROGRAM)
				LEFT JOIN UPN_RPT_DM_DEV.ODSMGR.COURSE_CATALOG D ON SFRREGP_TERM_CODE = D.ACADEMIC_PERIOD AND SFRREGP_SUBJ_CODE = D.SUBJECT AND SFRREGP_CRSE_NUMB_LOW = D.COURSE_NUMBER
		WHERE B.FECHA_INICIO = (SELECT MAX(B1.START_DATE) FROM SFRREGP A1 INNER JOIN LOE_SECTION_PART_OF_TERM B1 ON A1.SFRREGP_TERM_CODE = B1.TERM_CODE WHERE A1.SFRREGP_PIDM = A.SFRREGP_PIDM)
			--SFRREGP_TERM_CODE IN (SELECT TERM_CODE FROM LOE_SECTION_PART_OF_TERM WHERE CURRENT_DATE < END_DATE AND SUBSTR(TERM_CODE,4,1) IN (''4'',''5''))
		)
	...
	FROM (SELECT A.PROGRAM, A.TERM_CODE_EFF, A.AREA, A.AREA_RULE, A.SMBARUL_DESC, A.SEQ, A.COD_CURSO, A.CURSO, A.CREDITOS, A.REQUISITOS, A.EQUIVALENCIAS
		...
		FROM PLAN_ESTUDIOS A
				...
				LEFT JOIN UPN_RPT_DM_DEV.ODSMGR.LOE_REPEAT_COURSE_LCUR R ON B.PROGRAM = R.PROGRAM AND B.TERM_CODE_EFF = R.TERM_CODE_EFF AND B.PERSON_UID = R.PERSON_UID
