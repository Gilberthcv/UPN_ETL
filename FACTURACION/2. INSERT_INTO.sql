...
INSERT INTO PS_FACTURACION (BUSINESS_UNIT, PO_REF, PRODUCT, NIVEL, OPERATING_UNIT, CAMPUS, BILL_TO_CUST_ID, NOMBRE_COMPLETO, NOMBRE_COMPLETO_CORPORATE
	, BILL_TYPE_ID, INSTALL_NBR, INVOICE, LINE_SEQ_NUM, DESCR, SVRSVPR_SRVC_CODE, SVVSRVC_DESC, SVVSRVC_SRCA_CODE, PERIODO_CONTABLE, INVOICE_DT, FECHA_VENCIMIENTO
	, VAT_RGSTRN_ID, NET_EXTENDED_AMT, INVOICE_AMOUNT, ORIGINAL_INVOICE, ENTRY_TYPE_BI, ACCOUNT_BI, JOURNAL_ID_BI, JOURNAL_DATE_BI, REV_RECOG_BASIS_DESC
	, MONTO_AFECTO, MONTO_INAFECTO, MONTO_IMPUESTO, MONTO_TOTAL, SUBCATEGORIA, CATEGORIA, PERIODO, START_DATE, END_DATE, CARTERA_ACTIVA, FECHA_SINCRONIZACION)
SELECT
...
FROM (
	...
	FROM ( SELECT DISTINCT 
	        ...
	        FROM UPN_RPT_DM_PROD.ODSMGR.PS_BI_HDR A, UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE B, UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_DST C
	        ...
	        GROUP BY A.BUSINESS_UNIT, A.INVOICE, A.BILL_STATUS, A.BILL_TYPE_ID, A.INVOICE_AMOUNT, A.INVOICE_DT, A.PO_REF, A.TOT_VAT, A.DUE_DT
				...
				INNER JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_ACCT_ENTRY E ON D.BUSINESS_UNIT = E.BUSINESS_UNIT AND D.INVOICE = E.INVOICE AND D.LINE_SEQ_NUM = E.LINE_SEQ_NUM
				...
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_CUSTOMER I ON D.BILL_TO_CUST_ID = I.CUST_ID
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_CUSTOMER J ON I.CORPORATE_CUST_ID = J.CUST_ID
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_CUST_VAT_REG L ON D.BILL_TO_CUST_ID = L.CUST_ID
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_INSTALL_SCHE M ON D.INVOICE = M.GENERATED_INVOICE
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_NOTE N ON D.INVOICE = N.INVOICE AND N.TEXT254 LIKE '%ORIGINAL_INVOICE:%'
													    	AND N.LINE_SEQ_NUM = (SELECT MIN(N1.LINE_SEQ_NUM) FROM UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_NOTE N1
																				WHERE N1.BUSINESS_UNIT = N.BUSINESS_UNIT AND N1.INVOICE = N.INVOICE
																					AND N1.TEXT254 LIKE '%ORIGINAL_INVOICE:%')
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_NOTE N0 ON D.INVOICE = N0.INVOICE AND N0.NOTE_TYPE = 'INVOICE'
														AND N0.LINE_SEQ_NUM = (SELECT MIN(N2.LINE_SEQ_NUM) FROM UPN_RPT_DM_PROD.ODSMGR.PS_BI_LINE_NOTE N2
																				WHERE N2.BUSINESS_UNIT = N0.BUSINESS_UNIT AND N2.INVOICE = N0.INVOICE
																					AND N2.NOTE_TYPE = 'INVOICE')
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_BI_INSTALL_SCHE M0 ON NVL(SUBSTR(N.TEXT254,LENGTH(N.TEXT254)-9),N0.TEXT254) = M0.GENERATED_INVOICE
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_PAY_TRMS_NET P ON D.BUSINESS_UNIT = P.SETID AND D.PYMNT_TERMS_CD = P.PYMNT_TERMS_CD
														AND P.EFFDT = (SELECT MAX(P1.EFFDT) FROM UPN_RPT_DM_PROD.ODSMGR.PS_PAY_TRMS_NET P1
																		WHERE P1.SETID = P.SETID AND P1.PYMNT_TERMS_CD = P.PYMNT_TERMS_CD AND P1.EFFDT <= CURRENT_DATE)
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_PAY_TRMS_TIME Q ON P.SETID = Q.SETID AND P.PAY_TRMS_TIME_ID = Q.PAY_TRMS_TIME_ID
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_ITEM R ON D.BUSINESS_UNIT = R.BUSINESS_UNIT AND D.INVOICE = R.ITEM AND (R.CUST_ID <> '9999999999' OR R.CUST_ID IS NULL)
				LEFT JOIN ( SELECT DISTINCT ACCOUNT_UID, ID, NAME, CREDIT_CARD_NUMBER
								, SVRSVPR_SRVC_CODE, SVVSRVC_DESC, SVVSRVC_SRCA_CODE
							FROM UPN_RPT_DM_PROD.ODSMGR.RECEIVABLE_ACCOUNT_DETAIL, UPN_RPT_DM_PROD.ODSMGR.LOE_SVRSVPR, UPN_RPT_DM_PROD.ODSMGR.LOE_SVVSRVC
				...
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_BASE_TABLE_EQUIV T ON D.OPERATING_UNIT = T.VALUE1 AND T.TABLE_PARENT_ID = 1
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_BASE_TABLE_EQUIV U ON D.PRODUCT = U.VALUE1 AND U.TABLE_PARENT_ID = 9
				LEFT JOIN ( SELECT TERM_CODE, SUBSTR(STVTERM_DESC,1,6) AS SEMESTRE, STVTERM_DESC
							...
							FROM UPN_RPT_DM_PROD.ODSMGR.LOE_SECTION_PART_OF_TERM, UPN_RPT_DM_PROD.ODSMGR.STVTERM
				...
				LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.LOE_SECTION_PART_OF_TERM W ON D.PO_REF = W.TERM_CODE
	) Z
;

INSERT INTO PS_ANULACION (BUSINESS_UNIT, ITEM, ITEM_LINE, ITEM_SEQ_NUM, ENTRY_TYPE, ENTRY_REASON, ENTRY_AMT, ACCOUNTING_DT, PERIODO_CONTABLE, JOURNAL_ID_AR, JOURNAL_DATE_AR, FECHA_SINCRONIZACION)
SELECT DISTINCT F.BUSINESS_UNIT, F.ITEM, F.ITEM_LINE, F.ITEM_SEQ_NUM, F.ENTRY_TYPE, F.ENTRY_REASON, F.ENTRY_AMT, F.ACCOUNTING_DT
	, F.PERIODO_CONTABLE, G.JOURNAL_ID AS JOURNAL_ID_AR, G.JOURNAL_DATE AS JOURNAL_DATE_AR, CURRENT_TIMESTAMP
FROM ( SELECT BUSINESS_UNIT, ITEM, ITEM_LINE, ITEM_SEQ_NUM, ENTRY_TYPE, ENTRY_REASON, ENTRY_AMT, ACCOUNTING_DT, TO_CHAR(ACCOUNTING_DT,'YYYYMM') AS PERIODO_CONTABLE
        ...
        FROM UPN_RPT_DM_PROD.ODSMGR.PS_ITEM_ACTIVITY
        WHERE BUSINESS_UNIT = 'PER03' AND ENTRY_TYPE NOT IN ('PY','MT') --AND ACCOUNTING_DT BETWEEN TO_DATE(:FECHA_INICIO,'YYYY-MM-DD') AND TO_DATE(:FECHA_FIN,'YYYY-MM-DD')
			...
			LEFT JOIN UPN_RPT_DM_PROD.ODSMGR.PS_ITEM_DST G ON F.BUSINESS_UNIT = G.BUSINESS_UNIT AND F.ITEM = G.ITEM AND F.ITEM_LINE = G.ITEM_LINE AND F.ITEM_SEQ_NUM = G.ITEM_SEQ_NUM
